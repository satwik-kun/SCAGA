...
class CBCT_dataset(Dataset):
    def __init__(
        ...
        with open(os.path.join(self.data_root, 'meta_info.json'), 'r') as f:
            ...
            # Filter to keep only scans that exist locally
            valid_names = []
            for name in name_list:
                file_path = os.path.join(self.data_root, self.info['image'].format(name))
        ...
        # In-memory caches to prevent slow Windows I/O
        self._projs_cache = {}
        self._ct_cache = {}
        self._block_cache = {}
    ...
    def sample_projections(self, name, n_view=None):
        # -- load projections with in-memory caching
        if name in self._projs_cache:
            data = self._projs_cache[name]
        else:
            with open(os.path.join(self.data_root, self.info['projs'].format(name)), 'rb') as f:
                data = pickle.load(f)
            self._projs_cache[name] = data
    ...
    def load_ct(self, name):
        cache_key = (name, self.out_res_scale)
        if cache_key in self._ct_cache:
            return self._ct_cache[cache_key]
        ...
        self._ct_cache[cache_key] = (image, spacing, origin)
        return image, spacing, origin
    
    def load_block(self, name, b_idx):
        cache_key = (name, b_idx)
